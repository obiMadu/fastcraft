...
def init(
    ...
    if base_dir.exists():
        print(f"[red]❌ Error: A project named '{project_name}' already exists in the current directory.[/red]")
    ...
    transient=True 
        ...
        for task_func in steps:
            progress_task = progress.add_task("", total=None)  # No description, just the spinner
            task_func()
